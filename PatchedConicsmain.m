...
tf = juliandate(datetime('2026-01-01 00:00:00')); %Search end date
inc = 7; %Date search increment; ex: if inc = 7, searches every week
travmin = 4*30; %minimum allowed travel time
travmax = 10*30; %maximum allowed travel time
...
[re,rm,~] = EarthMarsSML1Positions(te,dt);
[retemp,rmtemp,~] = EarthMarsSML1Positions(te+1/24,dt); %Vel estimate
ve = (retemp-re)/(60*60);
vm = (rmtemp-rm)/(60*60);
...
% steps through calculations to find the total dV needed for the
% trajectory.
% NOTE: Due to time constaints, currently does not accurately calculate
% velocity. Currently, script finds position at a certain time and then the
% position 1 hour later, finds the difference and divides by 1 hour.
%% 0 Important parameters
GM_S = 1.32712440018E20/(1000^3); %All constants obtained from JPL Horizons
GM_E = 398600.435436;
Re = 6371.01;
...
%Gathers Earth and Mars heliocentric positions at the given times
[re,~,rSML1] = EarthMarsSML1Positions(te,dt);
[retemp,~,rSML1temp] = EarthMarsSML1Positions(te+1/24,dt); %Vel estimate
ve = (retemp-re)/(60*60);
vSML1 = (rSML1temp-rSML1)/(60*60);
%Open Source Lambert problem solver, available at https://github.com/rodyo/FEX-Lambert
%Finds needed velocities for desired trajectories
...
function [re,rm,rSML1] = EarthMarsSML1Positions(t0,dt)
...
[xe,ye] = kepler_2body(a_e,e_e,mue,t0,t_p_e); %Ecliptic with E at t0
[xm,ym] = kepler_2body(a_m,e_m,mum,t0+dt,t_p_m); %Ecliptic with M at t0+dt
...
function [x,y] = kepler_2body(a,e,mu,t,Tp) 
